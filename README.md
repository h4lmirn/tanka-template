...
右上の「Use this template」→「Create a new repository」で自分のリポジトリを作成します。  
（Forkでも使えます）

### 2. テーマを選ぶ（任意）

Actionsタブ → **「テーマを選ぶ（index.html を切り替え）」** から好きなテーマを選べます。  
既定では中立テーマが入っているので、この手順はあとからでも大丈夫です。
...
> このテンプレートは `main` に `index.html` が含まれているため、作成直後でもそのまま公開できます。
...
| テーマを選ぶ（index.html を切り替え） | 見た目のテーマを選択して反映 |
...
| shubi | 白地に朱赤と落款。書の美意識 |
| kasumi | 淡いローズ×モーヴ。春霞のような柔らかさ |
| shiro | 純白×グレー連番付き。モダンミニマル |

テーマを変えたい場合は Actions の **「テーマを選ぶ（index.html を切り替え）」** を使ってください。  
`preserve_config` を `true` のまま実行すると、現在の `CONFIG`（名前・Xハンドル・URL）を維持したまま見た目だけ変えられます。
